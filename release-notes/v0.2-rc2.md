# ngscopeclient v0.2-rc2 release notes

This release includes a few new drivers and filters, but is primarily focused on performance, stability, and improving the internal application architecture to be more maintainable in the future. It introduces GPU acceleration for many new filters, offering massive (1-3 order of magnitude) speedups on common operations.

## Changes since v0.2-rc1

* Update nativefiledialog-extended to latest
* Performance improvements to filter graph worload scheduling
* Performance improvements to Ethernet protocol decoding common path
* JitterSpectrumFilter: fixed regression causing crashes and incorrect timebase
* Use ImGui file browser on Wayland to prevent crashes caused by libdecor making non-thread-safe GTK calls even though we are trying to confine all GTK calls to the NFD worker thread
