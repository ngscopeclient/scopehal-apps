# ngscopeclient v0.2 release notes
...
* Core: Performance improvements to filter graph workload scheduling
...
* Filters: Performance improvements to Ethernet protocol decoding common path
...
* GUI: Use ImGui file browser on Wayland to prevent crashes caused by libdecor making non-thread-safe GTK calls even though we are trying to confine all GTK calls to the NFD worker thread
...
* Core: Update nativefiledialog-extended to latest
